...
func _draw() -> void:
	...
	var MidZoom = snapped(ZoomPerStep * (Steps / 2.0) + MinZoom, 0.1)
	
	var Lines : PackedVector2Array
	
	if (MidLinePos > 0 and MidLinePos < ContainerSize):
		Lines.append(Vector2(30 + Off, MidLinePos))
		Lines.append(Vector2(37 + Off, MidLinePos))
	...
	for g in range(1, Steps + 1):
		...
		if (UpOffset > 0 and UpOffset < ContainerSize):
			...
			Lines.append(Vector2(30 + Off, UpOffset))
			Lines.append(Vector2(37 + Off, UpOffset))
		...
		if (DownOffset > 0 and DownOffset < ContainerSize):
			var StepZoom = snapped(MidZoom + (ZoomPerStep * g), 0.1)
			Lines.append(Vector2(30 + Off, DownOffset))
			Lines.append(Vector2(37 + Off, DownOffset))
	...
	draw_multiline(Lines, Color("ffc315"), 2)
