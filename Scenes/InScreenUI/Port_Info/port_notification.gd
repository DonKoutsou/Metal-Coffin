...
func PortUpdated(NewAlt : float) -> void:
	if (CurrentShip.CurrentPort == null):
		...
		if (!ActionTracker.IsActionCompleted(ActionTracker.Action.HATCH)):
			...
			ActionTracker.QueueTutorial("Disembarkation", "You've landed in a town. To disembark and visit it press the [color=#ffc315]Open Hatch[/color] button.", [ScreenUI.UI_ELEMENT.HATCH_BUTTON])
