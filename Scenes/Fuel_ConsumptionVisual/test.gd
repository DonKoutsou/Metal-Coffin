...
@export var eff_Escalation = 2 :
	set(value):
		eff_Escalation = value
		queue_redraw()
@export var start_cap = 100 :
	set(value):
		start_cap = value
		queue_redraw()
@export var cap_Escalation = 6 :
	set(value):
		cap_Escalation = value
		queue_redraw()
@export var W = 10 :
	set(value):
		W = value
		queue_redraw()
@export var w_Escalation = 20 :
	set(value):
		w_Escalation = value
		queue_redraw()
...
func _draw():
	...
	var font_size = 16
	var g_width = size.x - 200
	var g_height = size.y - 100
	var origin_x = 40
	var origin_y = size.y - 60
	...
	for i in points:
		var FuelEf = start_eff * pow(eff_Escalation, float(i)/(points-1))  # doubles over range
		var FuelCap = start_cap * pow(cap_Escalation, float(i)/(points-1))
		var w =  W * pow(w_Escalation, float(i)/(points-1))
		
		var rng = MapShip.RangeFunction(FuelCap, FuelEf, w)
