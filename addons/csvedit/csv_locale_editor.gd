...
func _create_entry(p_codes: PackedStringArray = _header_data, 
	...
	var _locale_entry : EditorLocaleEntry = LocaleEntryScene.instantiate()
	
	_locale_entry.cell_picked.connect(EntryPicked)
	_locale_entry.cell_edited.connect(EntryEdited)
	_locale_entry.init(p_codes, p_entry_data)
...
func _on_text_edit_text_changed() -> void:
	_selected_entry.set_cell_text(_selected_index, editText.text)
	RichTextPreview.text = editText.text

func EntryPicked(entry : EditorLocaleEntry, index : int) -> void:
	_selected_entry = entry
	_selected_index = index
	EntryEdited(entry.get_cell(index).text)

func EntryEdited(newText : String) -> void:
	RichTextPreview.text = newText
	if (!editText.has_focus()):
...
func _save_csv(p_path: String) -> Error:
	...
	_dirty = false
	save_finished.emit(true)
	EditorInterface.get_editor_toaster().push_toast("File Saved!")
	return Error.OK
#endregion
