...
signal cell_edited(new_value: String)
signal cell_picked(cell : EditorLocaleEntry)
...
func init(p_codes: PackedStringArray, p_entries := PackedStringArray()) -> void:
	
	size_flags_horizontal = Control.SIZE_EXPAND_FILL

	_row_data.resize(p_codes.size())
	
	for i: int in p_codes.size():
		var code: String = p_codes[i]
		var _tedit := TextEdit.new()
		_tedit.custom_minimum_size = custom_size
		_tedit.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		
		_apply_tedit_settings(_tedit, code, i == 0)
		add_child(_tedit)
		...
		_tedit.gui_input.connect(textEditInput.bind(_tedit))
		_tedit.text_changed.connect(textEditEdited.bind(_tedit))
...
func textEditInput(event: InputEvent, edit : TextEdit) -> void:
	if (event is InputEventMouseButton and !event.is_echo() and event.is_pressed()):
		if (event.button_index == 1):
			cell_picked.emit(self, edit.get_index() - 1)

func textEditEdited(edit : TextEdit) -> void:
	cell_edited.emit(edit.text)
